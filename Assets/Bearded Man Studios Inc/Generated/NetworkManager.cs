...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CombatHandlerNetworkObject = null;
		...
		public GameObject[] PlayerHealthNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CombatHandlerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CombatHandlerNetworkObject.Length > 0 && CombatHandlerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CombatHandlerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is PlayerHealthNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerHealthNetworkObject.Length > 0 && PlayerHealthNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerHealthNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCombatHandler instead, its shorter and easier to type out ;)")]
		public CombatHandlerBehavior InstantiateCombatHandlerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CombatHandlerNetworkObject[index]);
			var netBehavior = go.GetComponent<CombatHandlerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CombatHandlerBehavior>().networkObject = (CombatHandlerNetworkObject)obj;
		...
		[Obsolete("Use InstantiatePlayerHealth instead, its shorter and easier to type out ;)")]
		public PlayerHealthBehavior InstantiatePlayerHealthNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerHealthNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerHealthBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerHealthBehavior>().networkObject = (PlayerHealthNetworkObject)obj;
		...
		public CombatHandlerBehavior InstantiateCombatHandler(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CombatHandlerNetworkObject[index]);
			var netBehavior = go.GetComponent<CombatHandlerBehavior>();
		...
		public PlayerHealthBehavior InstantiatePlayerHealth(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerHealthNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerHealthBehavior>();
